...
create table pharmacy(
	...
	buy integer,
	price integer,
	purchase_bill integer,
	selling_bill integer default 0,
	m_profit integer,
	m_loss integer,
	primary key(id)
);
...
before insert or update of target_sell,purchase_bill on pharmacy for each row
declare

begin
:new.target_sell:=:new.total*.8;
:new.purchase_bill:=:new.total*:new.buy;
end;
/
	...
	before insert or update of bill on buying for each row
        ...
        bill:=(medi_bill-medi_bill*(.25));
         ...
         :new.bill:=bill;
          ...
          update pharmacy set selling_bill=medi_bill where id=:new.id;
...
insert into buying(d_NHS,p_NHS,id,amount,b_date) values(3,2,1,5,'27/2/2018');
insert into buying(d_NHS,p_NHS,id,amount,b_date) values(4,4,1,1,'27/1/2018');
